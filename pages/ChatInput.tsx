...
export default function ChatInput() {
  ...
  const { mutate, isSuccess } = useMutation({
    ...
    onSuccess: async (resp: Response) => {
      ...
      queryClient.setQueryData<ChatResponse[]>(['chats'], (prev) => [
        ...(prev ?? []),
        {
          id: questionId,
          message: question,
      ...
      reader?.read().then(function processText({ done, value }): unknown {
        ...
        queryClient.setQueryData<ChatResponse[]>(['chats'], (prev) => [
          ...(prev?.slice(0, -1) ?? []),
          {
            ...prev?.at(-1),
            message:
              (prev?.at(-1)?.message ?? '') + new TextDecoder().decode(value),
          } as ChatResponse,
        ])
  ...
  return (
    <div className="border-t-2 border-gray-200 mb-2 sm:mb-0">
      <div className="relative flex">
        ...
        <div className="items-center inset-y-0 hidden sm:flex">
          ...
          <button
            ...
            onClick={handleSubmit(({ message }) => mutate(message))}
          >
